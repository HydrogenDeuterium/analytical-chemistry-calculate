# -*- coding: utf-8 -*-
"""
Created on Tue May  7 11:24:07 2019
版权所有（c）<2019> 氘化氢
...
# give data of a compound and return what the element and numbers of it
def normalize(compounddata):
    for j in range(len(compounddata)):
        if compounddata[j] in "1234567890":
            return [compounddata[:j], int(compounddata[j:])]
    return [compounddata, 1]


if __name__ == "__main__":
    #string of coupounds to be calculate
    compoundstring = input("print the compound to calculate its molarmass,enter to quit:\n")
    while compoundstring != "":
        splitstring = ""
        for i in compoundstring:
            if i in "QWERTYUIOPASDFGHJKLZXCVBNM":
                splitstring += " "
            splitstring += i
        compoundlist = splitstring.split()
        compounddict = dict(map(normalize, compoundlist))
        s = 0
        try:
            for i in compounddict:
                s += data.element_molarmass[i] * compounddict[i]
        except KeyError:
                print("inputError!")
        else:
            pass
